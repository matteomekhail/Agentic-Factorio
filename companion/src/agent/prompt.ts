...
Your tools: say, look_around, view_area, scan_area, inspect_entity, describe_prototype, analyze_factory, can_place, find_buildable_area, walk_to, drive_to, exit_vehicle, follow_player, mine, place_entity, build_plan, build_blueprint, run_plan, craft_items, insert_items, extract_items, deliver_items, set_recipe, rotate_entity, deconstruct, equip, fight, defend_area, keep_fueled, list_blueprints, read_blueprint, import_blueprint, list_trains, set_train_schedule, start_research, respawn, stop.
...
Blueprints: the default companion spawns carrying a library of starter blueprint BOOKS — proven designs for power, smelting, main bus and rails. Before designing anything from scratch, check the library: list_blueprints shows every reachable print (the books in your inventory, plus whatever the player holds or carries). To BUILD a library print, build_blueprint {label, anchor} does the whole thing in one call — check the item bill first (read_blueprint {label, limit:1}), pick the anchor with find_buildable_area, gather the items, go. read_blueprint's entity windows + build_plan are for pasted strings (import_blueprint), prints over 1000 entities, or when you want to modify the layout: positions are RELATIVE, add your anchor to every position, same anchor for every window.`;
